...
**配置热重载**，**AI 助手**（RAG + LLM）支持智能问答、配置生成、日志诊断与数据洞察，支持在企业微信、Telegram 等推送平台中直接对话，开箱即用。
...
| 雨云签到     | `rainyun`           | 08:30        | ✅         | `enable: true` 且配置 accounts（username+password）时执行；支持多账号；api_key 可选用于续费；ddddocr 验证码识别；**服务器到期检查与自动续费**（参考 Rainyun-Qiandao） |
...
> - **雨云签到** 验证码检测依赖 OpenCV（项目已依赖 `opencv-python-headless`）。若出现「module 'cv2' has no attribute 'imdecode'」或「当前环境中的 cv2 缺少 imdecode」，请先尝试在项目目录执行 `uv pip install --force-reinstall opencv-python-headless`（可能是安装不完整）；若曾安装过占位包 `cv2`，请执行 `pip uninstall cv2`。
...
配置管理、任务管理、数据展示、日志查看，以及 **AI 助手**（配置生成、日志诊断、数据洞察、可执行操作，需在 `config.yml` 中启用 `ai_assistant`）。默认账号 `admin` / `123`（生产环境请修改）。
...
基于 **RAG（检索增强生成）+ LLM** 的智能对话能力，支持配置生成、日志诊断、数据洞察及可执行操作，可在 **Web 界面**与 **企业微信、Telegram** 等推送平台中直接对话。
...
<details>
<summary><strong>📷 展开查看 AI 助手与企业微信交互截图</strong></summary>

**Web 界面**：功能介绍 | 修改配置 | 执行任务 | 查询数据

<table>
<tr>
<td><img src="web/static/AI助手功能介绍.png" alt="AI助手功能介绍" width="100%"></td>
<td><img src="web/static/AI助手修改配置.png" alt="AI助手修改配置" width="100%"></td>
<td><img src="web/static/AI助手执行任务.png" alt="AI助手执行任务" width="100%"></td>
<td><img src="web/static/AI助手查询数据.png" alt="AI助手查询数据" width="100%"></td>
</tr>
</table>
...
<table>
<tr>
<td><img src="web/static/企业微信后台介绍.jpg" alt="企业微信后台介绍" width="100%"></td>
<td><img src="web/static/企业微信查询数据1.jpg" alt="企业微信查询数据1" width="100%"></td>
<td><img src="web/static/企业微信查询数据2.jpg" alt="企业微信查询数据2" width="100%"></td>
<td><img src="web/static/企业微信查询数据3.jpg" alt="企业微信查询数据3" width="100%"></td>
</tr>
</table>
...
📖 **完整说明**（企业微信/Telegram 接入步骤、配置项详解、数据来源）见 **[AI 助手使用指南](docs/guides/ai-assistant.md)**。
...
本项目在开发过程中参考或借鉴了以下开源项目与作者的工作，在此致以感谢：

| 项目 / 作者 | 说明 |
|:------------|:-----|
| [aio-dynamic-push](https://github.com/nfe-w/aio-dynamic-push)（[@nfe-w](https://github.com/nfe-w)） | 推送与任务调度参考 |
