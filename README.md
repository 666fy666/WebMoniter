...
# 方法2：重新创建容器
docker compose up -d --force-recreate web-monitor
```
...
**Q: 如何更新 Cookie？**  
A: 直接修改 `config.yml` 中的 Cookie 值，然后重启容器（Docker 部署）或重启程序（本地部署），系统会在下次执行监控任务时自动重新加载配置。
...
**Q: 如何调整监控频率？**  
A: 在 `config.yml` 的 `scheduler` 部分修改对应的间隔时间（秒）。修改后需要重启服务。
...
- **Cookie 管理**：定期更新 Cookie，失效时会记录错误日志。建议每月更新一次
