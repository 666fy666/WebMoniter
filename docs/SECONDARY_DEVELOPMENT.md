...
2. **补充配置映射**：若用顶层配置，在 `src/config.py` 中增加 `AppConfig` 字段，并在 `src/config_loader_specs.py` 中补充 YAML 到扁平字段的映射规格。
3. **实现任务逻辑**：一个无参的 async 入口函数（内部 `get_config(reload=True)`、业务逻辑、可选推送）。
4. **注册**：在任务模块末尾调用 `register_monitor` 或 `register_task`，并在 `src/job_registry.py` 的 `MONITOR_MODULES` / `TASK_MODULES` 中追加模块路径。
...
- `src/config_loader_specs.py`：`config.yml` 节点到 `AppConfig` 扁平字段的映射，以及多账号、多 Cookie/Token 字段规格。
...
在 `src/config_loader_specs.py` 的 `CONFIG_MAPPINGS` 中补充 `checkin` 节点到 `AppConfig` 扁平字段的映射；如果支持 `accounts` 多账号，再在 `MULTI_ACCOUNT_SPECS` 中补充对应规格。`load_config_from_yml()` 会统一按这些规格写入 `config_dict`。
...
小结：顶层定时任务 = **config.yml 节点 → AppConfig + config_loader_specs 映射 → 任务模块（run_xxx_once + 推送 + _get_xxx_trigger_kwargs）→ register_task → TASK_MODULES 一行**。
...
- **src/config_loader_specs.py**：在 `CONFIG_MAPPINGS["huya"]` 中声明从 `yml_config["huya"]` 到 `AppConfig` 扁平字段的映射，`load_config_from_yml()` 会统一处理。
...
见 `src/config.py` 中 `HuyaConfig`、`AppConfig.get_huya_config`，以及 `src/config_loader_specs.py` 中的 huya 映射段落。
...
小结：监控任务 = **config.yml（业务节点含 enable + scheduler 间隔）→ AppConfig + config_loader_specs 映射 + get_xxx_config → 继承 BaseMonitor 实现 run + 推送 → run_xxx_monitor + _get_xxx_trigger_kwargs → register_monitor → MONITOR_MODULES 一行**。`enable: false` 时任务会被暂停，热重载生效。
...
|----------|------------|----------|----------|----------------|------|
| 定时任务 | iKuuu 签到 | `config.yml` → `checkin` | `AppConfig` + `CONFIG_MAPPINGS` / `MULTI_ACCOUNT_SPECS` | `tasks/ikuuu_checkin.py`（`run_checkin_once`、`_send_checkin_push`、`_get_checkin_trigger_kwargs`） | `register_task("ikuuu_checkin", ...)`，`TASK_MODULES` 含 `tasks.ikuuu_checkin` |
...
| 定时任务 | Freenom 续期 | `config.yml` → `freenom` | `AppConfig` + `CONFIG_MAPPINGS` / `MULTI_ACCOUNT_SPECS` | `tasks/freenom_checkin.py`（`run_freenom_checkin_once`、`_get_freenom_trigger_kwargs`） | `register_task("freenom_checkin", ...)`，`TASK_MODULES` 含 `tasks.freenom_checkin` |
| 定时任务 | 天气推送   | `config.yml` → `weather` | `AppConfig` + `CONFIG_MAPPINGS` | `tasks/weather_push.py`（`run_weather_push_once`、`_get_weather_trigger_kwargs`） | `register_task("weather_push", ...)`，`TASK_MODULES` 含 `tasks.weather_push` |
| 监控任务 | 虎牙监控   | `config.yml` → `huya`（含 `monitor_interval_seconds`） | `AppConfig`、`HuyaConfig`、`get_huya_config`、`CONFIG_MAPPINGS` | `monitors/huya_monitor.py`（`HuyaMonitor`、`run_huya_monitor`、`_get_huya_trigger_kwargs`） | `register_monitor("huya_monitor", ...)`，`MONITOR_MODULES` 含 `monitors.huya_monitor` |
...
- [ ] 若用顶层配置：在 `AppConfig` 中补充字段，并在 `src/config_loader_specs.py` 中补充映射规格；若用 `plugins`，无需改 config.py。
...
- [ ] 在 `AppConfig` 中增加扁平字段，在 `src/config_loader_specs.py` 中补充映射规格；可选：提供 `get_my_monitor_config()` 返回结构化配置。热重载通过 `model_dump()` 自动覆盖所有字段，无需手动维护比较列表。
