...
- **AI 助手**：RAG + LLM 智能问答，支持配置生成、日志诊断、数据洞察及可执行操作（开关监控、配置列表增删）
...
**关键代码流程**（`main.py` 负责编排，`src/jobs/lifecycle.py` 封装 Web/调度/AI 细节）：
```python
async def main():
...
**职责**：Web 服务、任务注册与首轮执行、配置热重载回调、AI 向量库构建等启动/关闭细节（由 `main.py` 调用）。
...
- `maybe_build_ai_rag_index()`：AI 助手启用时在启动阶段构建 Chroma 索引
...
- `GET /api/assistant/status`：获取 AI 助手状态（需登录；未登录返回 `enabled: false`）
...
│   │   ├── lifecycle.py    # Web/调度/AI 启动编排与热重载回调
...
│   ├── push_channel/       # 推送通道（17 种 type，含 demo、qlapi）
│   │   ├── _push_channel.py
│   │   ├── manager.py
...
│   │   ├── config_io.py
│   │   ├── data_support.py
│   │   ├── assistant_support.py
│   │   └── routers/        # pages、auth、tasks、config、data、logs、assistant、webhooks
│   ├── webUI/              # 前端静态资源与 Jinja2 模板
│   │   ├── static/         # css、js、截图等
│   │   └── templates/      # login、config、tasks、data、logs（dashboard.html 暂无路由）
│   ├── ai_assistant/       # RAG + LLM 助手
│   │   ├── config.py、rag.py、indexer.py、conversation.py
│   │   ├── intent_parser.py、tools.py、llm_client.py
│   │   ├── platform_chat.py、wecom_crypto.py
│   │   └── platform_handlers/  # wecom.py、telegram.py
│   └── ql/                 # 青龙单任务脚本与环境变量兼容
...
│   └── guides/             # config、tasks、web-ui、ai-assistant、push-channels 等
│
├── data/                   # 运行时数据（gitignore）
│   ├── data.db             # SQLite（监控数据 + task_run_history）
│   ├── cookie_cache.json
│   ├── auth.json           # Web 登录凭据
│   ├── ai_assistant_conversations.json
│   └── ai_assistant_chroma/ # Chroma 向量库（AI 启用时）
│
...
- `name`：主播名称
- `is_live`：直播状态
- `room_pic`、`avatar_url`：封面与头像（迁移字段，可选）（"1"=直播中，"0"=未开播）
...
> 该表由 `src/jobs/tracker.py` 在首次使用时创建，用于实现"当天已运行则跳过"功能。任务包装函数在 `run_func` 正常返回后写入记录；若抛出未捕获异常则不写入，允许后续重试。Web「立即运行」使用 `original_run_func`，不经过此检查。
