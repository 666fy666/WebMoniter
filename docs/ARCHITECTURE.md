...
### 5. src/jobs/task_outcome.py - 定时任务执行结果

**职责**：约定定时任务 `run_func` 的返回值语义，供 `register_task` 包装层判断是否写入「今日已运行」。

**约定**：
- `TASK_SUCCESS`（`True`）：任务已成功完成，当天可跳过
- `TASK_FAILED`（`False`）：未执行、配置不完整或执行失败，允许当天重试

定时任务实现应返回上述常量，而非依赖「正常 return 即视为成功」。

---

### 6. src/core/runtime.py - 运行时与退出兜底
...
### 7. src/storage/database.py - 数据库模块

**职责**：MySQL 权威主库、SQLite 本地镜像与自动故障回退
...
### 9. src/settings/db_sync.py - 配置与数据库同步
...
### 10. src/jobs/lifecycle.py - 生命周期编排
...
### 11. src/settings/watcher.py - 配置监控器
...
### 12. src/storage/cookie_cache.py - Cookie缓存管理
...
- `src/web/templating.py`：共享 Jinja2 模板环境
- `src/web/static_files.py`：`CachedStaticFiles`（微博图片等静态资源 Cache-Control）
- `JobDescriptor.description`：任务 ID 到展示文案（在 `register_*` 时注册）

**静态资源**：
- `/static` → `src/webUI/static/`
- `/weibo_img` → `data/weibo/`（带缓存头的 `CachedStaticFiles`）
...
### 15. src/jobs/log_manager.py - 日志管理
...
4. 执行任务逻辑
   ...
   ↓
5. 标记为已运行
   - 任务函数返回 `TASK_SUCCESS`（见 `src/jobs/task_outcome.py`）后写入 `task_run_history`
   - 返回 `TASK_FAILED`（未启用、配置不完整、执行失败等）时不写入，允许当天重试
   - 若抛出未捕获异常则不写入
   ↓
6. 推送结果
   - 构建推送消息
...
async def run_new_task():
    config = get_config()
    # 任务逻辑
    return TASK_SUCCESS  # 或 TASK_FAILED，见 src/jobs/task_outcome.py
