...
1. 在青龙面板 → **订阅管理** → **添加订阅**  
   ...
   - 定时规则：`0 0 * * *`（每日 0 点拉取一次）  
   - 需保留完整项目代码；如果使用白名单，请至少包含 `src/`、`pyproject.toml`、`uv.lock`
...
通过订阅或 `git clone` 获取完整仓库。统一 CLI 会导入注册表、任务模块、配置与推送模块，因此不能只保留 `src/ql/`。
...
- **雨云**：单账号可用 `WEBMONITER_RAINYUN_USERNAME`、`WEBMONITER_RAINYUN_PASSWORD`、`WEBMONITER_RAINYUN_API_KEY`，多账号使用 `WEBMONITER_RAINYUN_ACCOUNTS=[{"username":"u1","password":"p1","api_key":"可选"}]`
- **阿里云盘**：单账号用 `WEBMONITER_ALIYUN_REFRESH_TOKEN`，多账号用 `WEBMONITER_ALIYUN_REFRESH_TOKENS=token1,token2`
