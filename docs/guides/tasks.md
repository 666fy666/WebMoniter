...
- **免打扰时段**（`quiet_hours`）：在指定时间段内不推送通知，任务照常执行。详见 [定时任务详解 - 免打扰时段](tasks/checkin.md#quiet-hours)。  
...
若已使用 [青龙面板](https://github.com/whyour/qinglong)，可通过统一 CLI `python -m src.ql <task_id>` 运行定时任务，配置使用**环境变量**，推送走**青龙内置通知**（QLAPI）。详见 [青龙面板兼容指南](../QINGLONG.md)。
