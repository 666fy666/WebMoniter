...
def _run_xingkong_sync(username: str, password: str) -> tuple[bool, str]:
    try:
        ...
        r1 = session.post(LOGIN_URL, data={"username": username, "password": password, "remember": 1}, timeout=15)
        r1.raise_for_status()
        ...
        return True, "登录: %s; 签到: %s" % (login_msg, sign_msg)
...
async def run_xingkong_checkin_once() -> None:
    ...
    class XingkongConfig:
        ...
        def from_app_config(cls, config: AppConfig) -> "XingkongConfig":
            ...
            return cls(
                ...
                username=u, password=p, accounts=accounts,
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
        push_manager = await build_push_manager(
            ...
            init_fail_prefix="星空代理：", channel_names=cfg.push_channels or None,
        )
            ...
            if push_manager and not is_in_quiet_hours(app_config):
                try:
                    await push_manager.send_news(
                        ...
                        to_url="https://www.xkdaili.com", picurl="", btntxt="打开",
                    )
                except Exception:
                    pass
        if push_manager:
...
register_task("xingkong_checkin", run_xingkong_checkin_once, lambda c: {"minute": parse_checkin_time(getattr(c, "xingkong_time", "07:30") or "07:30")[1], "hour": parse_checkin_time(getattr(c, "xingkong_time", "07:30") or "07:30")[0]})
