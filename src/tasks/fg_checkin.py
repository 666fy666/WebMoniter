    ...
    class FgConfig:
        ...
        def from_app_config(cls, config: AppConfig) -> FgConfig:
            ...
            return cls(
                ...
                cookie=single,
                cookies=normalized_string_items(getattr(config, "fg_cookies", None), single),
                time=(getattr(config, "fg_time", None) or "00:01").strip() or "00:01",
                push_channels=task_push_channels(config, "fg_push_channels"),
            )
    ...
    async with push_manager_context(
        ...
        for idx, cookie_str in enumerate(effective):
            ...
            await send_news_if_allowed(
                ...
                app_config,
                logger,
                quiet_log="富贵论坛签到：免打扰时段，不发送推送",
                error_log="富贵论坛签到：推送失败 %s",
                title=title,
                description=body,
                to_url=FG_HOME_URL,
                picurl="",
...
def _get_fg_trigger_kwargs(config: AppConfig) -> dict:
    return cron_kwargs_from_config(config, "fg_time", "00:01")
