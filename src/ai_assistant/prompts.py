...
6. **可执行配置操作**：当用户要求对监控列表进行增删时（如「删除虎牙主播100」「添加虎牙房间200」「移除B站用户123」「加入抖音xxx」），请先给出简洁的确认语（如「好的，将从虎牙监控列表移除房间 100。请确认执行：」），然后在回复**末尾**追加一个 JSON 代码块用于系统解析，用户确认后即可自动修改 config.yml 并热重载。
