...
from src.core.runtime import arm_shutdown_watchdog
...
def _is_task_source(name: str) -> bool:
    return "Monitor" in name or name.startswith("src.tasks.") or name == "__main__"
...
class TaskGroupFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        with self._lock:
            ...
            need_sep = _is_task_source(name) and (
                prev is None or _is_push_channel(prev) or (_is_task_source(prev) and prev != name)
            )
